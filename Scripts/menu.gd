...
func generate():
	...
	for x in $"..".to_save["nations"][nation]:
		create_entry()
	for p in $Base/ScrollContainer/VBoxContainer.get_children():
		p.path.text = $"..".to_save["nations"][nation][str(p.get_index()+1)][0]
		p.notes.text = $"..".to_save["nations"][nation][str(p.get_index()+1)][1]
	if $Base/ScrollContainer/VBoxContainer.get_child_count() >= 1:
		size.y = 1
	$Base/Main/Base/HBoxContainer/Label2.text = str(len($"..".to_save["nations"][nation]))
...
func remove_entry():
	if len($"..".to_save["nations"][$"..".selected_nation.name]) >= 1:
		$"..".to_save["nations"][$"..".selected_nation.name].erase(str(len($"..".to_save["nations"][$"..".selected_nation.name])))
		$Base/ScrollContainer/VBoxContainer.get_child($Base/ScrollContainer/VBoxContainer.get_child_count()-1).queue_free()

func update_visuals():
	$Base/Main/Base/HBoxContainer/Label2.text = str(len($"..".to_save["nations"][$"..".selected_nation.name]))
	$"..".colour_nations()

func increase_log() -> void:
	create_entry()
	$"..".to_save["nations"][$"..".selected_nation.name][str(len($"..".to_save["nations"][$"..".selected_nation.name])+1)] = ["",""]
	update_visuals()
